...
/*************************************************************************/
    ...
    for(int t = 0; t < testcycles; t++)
    {
        stingycm = new StingyCM_Base(w_salsa, 5, 100);
        for(int i = 0; i < package_num; i++)
        {
            stingycm->Insert(insert[i], KEY_LEN);
        }
    }
    clock_gettime(CLOCK_MONOTONIC, &time2);
    resns = (long long)(time2.tv_sec - time1.tv_sec) * 1000000000LL + (time2.tv_nsec - time1.tv_nsec);
    double throughput_stingy = (double)1000.0 * testcycles * package_num / resns;
    printf("throughput of Stingy (insert): %.6lf Mips\n", throughput_stingy);
    ...
    for(int t = 0; t < testcycles; t++)
    {
        salsacm = new SalsaCM(w_salsa, 5, 100);
        for(int i = 0; i < package_num; i++)
        {
    ...
    printf("throughput of Transformer (insert): %.6lf Mips\n", throughput_dripsketch); 
    //printf("%.6lf\n", throughput_dripsketch);
...
/*************************************************************************/
    ...
    for(int t = 0; t < testcycles; t++)
    {
        for(int i = 0; i < flow_num; i++)
        {
            res_tmp = stingycm->Query(query[i], KEY_LEN);
        }
    }
    clock_gettime(CLOCK_MONOTONIC, &time2);
    resns = (long long)(time2.tv_sec - time1.tv_sec) * 1000000000LL + (time2.tv_nsec - time1.tv_nsec);
    throughput_stingy = (double)1000.0 * testcycles * flow_num / resns;
    printf("throughput of Stingy (query): %.6lf Mips\n", throughput_stingy);
    sum += res_tmp;
    //printf("%.6lf ", throughput_stingy);
    ...
    printf("throughput of Transformer (query): %.6lf Mips\n", throughput_dripsketch);
    sum += res_tmp;
    //printf("%.6lf\n", throughput_dripsketch);
...
/********************************************************************************************/
    ...
    double re_cm = 0.0, re_stingy = 0.0,  re_a = 0.0,  re_pcusketch = 0.0, re_pcsketch = 0.0, re_bmatcher = 0.0, re_elastic=0.0, re_nitro=0.0, re_salsacm=0.0, re_drip=0.0, re_tower=0.0;
    double re_cm_sum = 0.0, re_stingy_sum = 0.0,  re_a_sum = 0.0,  re_pcusketch_sum = 0.0, re_bmatcher_sum = 0.0, re_elastic_sum=0.0, re_nitro_sum=0.0, re_salsacm_sum=0.0, re_drip_sum=0.0, re_tower_sum=0.0;
    
    double ae_cm = 0.0, ae_stingy = 0.0,  ae_a = 0.0,  ae_pcusketch = 0.0, ae_bmatcher = 0.0, ae_elastic=0.0, ae_nitro=0.0, ae_salsacm=0.0, ae_drip=0.0, ae_tower=0.0;
    double ae_cm_sum = 0.0, ae_stingy_sum = 0.0,  ae_a_sum = 0.0,  ae_pcusketch_sum = 0.0, ae_bmatcher_sum = 0.0, ae_elastic_sum=0.0, ae_nitro_sum=0.0, ae_salsacm_sum=0.0, ae_drip_sum=0.0, ae_tower_sum=0.0;

    double val_cm = 0.0, val_stingy = 0.0,  val_a = 0.0,  val_pcusketch = 0.0, val_bmatcher = 0.0, val_elastic=0.0, val_nitro=0.0, val_salsacm=0.0, val_drip=0.0, val_tower=0.0;
    double erro_cm = 0.0, erro_stingy = 0.0, erro_a = 0.0,  erro_pcusketch = 0.0, erro_bmatcher = 0.0, erro_elastic=0.0, erro_nitro=0.0, erro_salsacm=0.0, erro_drip=0.0, erro_tower=0.0;
    ...
    for(unordered_map<string, int>::iterator it = unmp.begin(); it != unmp.end(); it++)
    {
        ...
        val_stingy = stingycm->Query(temp, KEY_LEN)*1.0;    
	    val_salsacm = salsacm->Query(temp, KEY_LEN)*1.0;
	    ...
	    re_salsacm = fabs(val_salsacm - val) / (max(val,1) * 1.0);
	    ...
	    ae_salsacm = fabs(val_salsacm-val);
